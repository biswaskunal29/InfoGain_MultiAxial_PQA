...
#    print(len(samples))
    ...
    profile_collection = np.zeros((0,384),dtype = np.single)
...
#        get all data        
        filename = sample + part2 + '.npy' 
        filepath = Path(class_path,sample,filename)
        dp_ocr = np.load(filepath)
        
        filename = sample + part3 + '.npy' 
        filepath = Path(class_path,sample,filename)
        banner_ocr = np.load(filepath)        
        
        filename = sample + part5 + '.npy' 
        filepath = Path(class_path,sample,filename)
        dp_labels = np.load(filepath)       
        
        filename = sample + part6 + '.npy' 
        filepath = Path(class_path,sample,filename)
        banner_labels = np.load(filepath) 
...
#        print(image_ocr.shape)        
        ...
        if(dp_ocr.size != 0):
            profile_collection = np.vstack((profile_collection,dp_ocr))
            
        if(banner_ocr.size != 0):
            profile_collection = np.vstack((profile_collection,banner_ocr))

        if(dp_labels.size != 0):
            profile_collection = np.vstack((profile_collection,dp_labels))

        if(banner_labels.size != 0):
            profile_collection = np.vstack((profile_collection,banner_labels))
        
        if(profile_desc.size != 0):
            profile_collection = np.vstack((profile_collection,profile_desc))
#            print(ocr_collection.shape)
            
        
#        print(j)
    
    print(profile_collection.shape)    
